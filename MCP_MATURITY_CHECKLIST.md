...
- [x] 在既有 required 语义上增加可选服务等待预算，允许慢服务在后续使用边界加入。
- [x] 用配置、工作区、服务及认证身份约束目录缓存；容量、TTL、失效和清理有界。
  缓存只作为派生数据，不能授予调用权限或表示连接已经就绪。
- [x] 若复用目录，调用绑定明确版本；不得改变已冻结 Turn 的工具和审批语义。
- [x] 在现有 owner 下合并预热请求，覆盖配置变化、账号切换、工作区退休、取消、
  required 失败及旧结果迟到，不额外创建后台生命周期所有者。
- [x] 用多个真实本地慢/失败服务记录启动耗时和目录可用性，证明改善及关闭正确性。
...
阶段五 Windows 源码 TTY 通过：同一组两个慢 HTTP 服务、一个失败服务和一个 required
stdio 服务，完整等待 3.17 秒，0.1 秒可选预算时约 0.53 秒返回；required 仍完整等待。
缓存调用等待原连接、定义变化阻断请求、冻结范围不增补工具、退休阻止迟到发布均通过；
最终运行 12 个已建立会话全部关闭。源码 TUI 验证后台启动提示、`/mcp status`、`/tools`
从 starting 到 ready 的一致性及 Ctrl+D 退出；操作系统确认本轮 30 个 fixture 进程均已退出。
523 项定向回归、59 项身份与生命周期复验、14 项审批链复验、139 项架构审计及文档检查通过。
账号切换及发送排队竞争使用真实 SDK 和受控凭据/HTTP 边界测试，不记作 Sentry 真机结果。
验收入口为 `python -m tests.manual.mcp_startup_acceptance`（`--client` 启动源码 TUI），
报告位于 `build/mcp-maturity-phase5-20260917/ACCEPTANCE.md`。本阶段实机证据限 Windows 本地服务，
跨平台与第三方综合验收继续保留在阶段七。

## 阶段六：调用中的表单与 URL 交互

- [x] 定义完整的本地 elicitation 用例：SDK 边界校验请求后转换为具名类型，
  通过现有交互队列呈现表单/URL 并返回 accept、decline 或 cancel。
- [x] 只有实际支持的交互类型才向服务声明能力；非交互入口返回明确结果。
- [x] 绑定调用身份与请求身份，处理并行请求、用户等待期限、服务端取消、
  连接关闭和调用方取消，不重放已产生效果的工具调用。
- [x] 明确本地交互与 AppServer 审批的职责；涉及线上消息时先遵守正式契约，
  不通过自造字段绕过远端审批。
- [x] 覆盖 schema 校验、敏感字段展示、URL 处理、所有终态及资源清理，
  使用确实提供对应能力的服务执行终端验收。

参考：`codex/codex-rs/codex-mcp/src/elicitation.rs`、
`codex/codex-rs/rmcp-client/src/elicitation_client_service.rs`。

阶段六 Windows 源码 TTY 通过：真实 stdio/HTTP MCP 服务驱动生产 TUI 和交互队列，
完成编辑后提交、拒绝、Esc 取消、并行表单排队、URL 拒绝不导航、同意后真实浏览器到达、
调用取消清理和另一连接继续交互，共 8 项。最终运行 8 次工具调用，无重放，2 个会话全部关闭；
操作系统确认两轮验收的 4 个服务进程均已退出。
731 项定向回归、195 项补充回归、60 项输入边界复验、14 项调用身份与审批链复验、
13 项最终视图清理复验通过；架构审计 138 项通过后修正验收脚本路径定位，剩余 1 项复验通过。
文档生成前后校验及差异检查通过。真实 SDK 自动化另覆盖 SSE 与非交互能力声明；
服务端取消、超时、过载及迟到答案由受控协议边界验证，不记作第三方真机结果。
验收入口为 `python -m tests.manual.mcp_elicitation_acceptance --directory <新目录>`，
...
- [x] 每阶段先跑受影响的定向测试，复核错误脱敏、类型契约、取消和 owner 关闭。
- [x] 包边界变更或最终收口执行架构审计；同步受影响稳定文档与文档生成校验。
- [x] 源码真实终端验收 HTTP、SSE、stdio 与 Sentry OAuth；危险和超限场景使用
  隔离本地服务，报告与真实第三方服务结果分开记录。
- [ ] 继续完成 `MCP_OAUTH_CHECKLIST.md` 中未完成的真实故障与跨平台验收。
  没有实际执行的平台和场景保留待验收。
- [x] 复核后按阶段提交和推送；交付列出通过证据、未完成事项和实际运行入口。

阶段七 Windows 源码验收通过已执行范围：15 项真实 PTY 本地 OAuth 回调、12 项真实
stdio/HTTP/SSE 传输检查、源码入口与系统凭据库检查。606 项 MCP 回归、30 项撤销证据
及文档定向复验、139 项架构审计通过；文档生成前后校验和差异检查通过。
补齐 SSE 验收入口，修正截止时间用例对调度相关重试次数的固定假设。

Sentry 本轮独立浏览器授权、只读调用和新进程恢复通过；真实服务端撤销后下一调用被拒绝，
工具目录清空，源码 TUI 明确提示重新授权。菜单停止、启动、重启、状态和工具展示通过；
logout、重复 logout、新进程未登录边界及 Ctrl+D 关闭通过，测试凭据和注册已清理。
完整模型回合因模型服务返回服务器过载而未触发本轮工具审批，不将其记为通过；此前审批与
菜单样式证据仍见既有报告。报告位于 `build/mcp-maturity-phase7-20260917/ACCEPTANCE.md`。

阶段七仍有待验收项：真实浏览器启动失败后的恢复、真实断网恢复、Sentry 刷新交换中的
logout 竞争，以及 Linux/macOS。当前机器无可用 WSL 或 macOS 环境，不能宣称全平台收口。
