...
- [ ] 由凭据与连接边界生成最小类型化状态，区分未知、显式 Header/Bearer、OAuth、
  未登录、需要重新授权；明确本地凭据存在与远端认证成功的区别。
- [ ] CLI、`/mcp`、`/tools` 消费同一事实来源；TUI 不直接访问凭据库或发起探测。
- [ ] 在失败服务旁给出针对该注册的登录/重启提示；普通启动不自动打开浏览器。
- [ ] 覆盖登录替换、logout、刷新失败、匿名服务、显式认证及零工具服务。
- [ ] 真机核对 Sentry 首次登录、认证失效和恢复后的展示一致性。
