...
const meriyahPromise = import("./vendor/meriyah.umd.min.js").then(
  (m) => m.default ?? m,
);
...
const internalBindingSalt = (() => {
  const raw = process.env.JS_REPL_SESSION_ID ?? "";
...
const tmpDir = process.env.JS_REPL_TMP_DIR || cwd;
const homeDir = process.env.HOME ?? null;
const nodeModuleDirEnv = process.env.JS_REPL_NODE_MODULE_DIRS ?? "";
...
function getRequireForBase(base) {
  ...
  if (!req) {
    req = createRequire(path.join(base, "__js_repl__.cjs"));
...
function nextInternalBindingName() {
  ...
  return `__repl_internal_commit_${internalBindingSalt}_${internalBindingCounter++}`;
}
...
async function buildModuleSource(code) {
  ...
  helperDeclarations.push(
    ...
    `const ${markCommittedFnName} = import.meta.__replInternalMarkCommittedBindings;`,
    `const ${markPreludeCompletedFnName} = import.meta.__replInternalMarkPreludeCompleted;`,
    "delete import.meta.__replInternalMarkCommittedBindings;",
    "delete import.meta.__replInternalMarkPreludeCompleted;",
  );
...
function encodeByteImage(bytes, mimeType, detail) {
  if (bytes.byteLength === 0) {
    throw new Error("host.emitImage expected non-empty bytes");
  }
  if (typeof mimeType !== "string" || !mimeType) {
    throw new Error("host.emitImage expected a non-empty mimeType");
  }
...
function parseImageDetail(detail) {
  ...
  if (typeof detail !== "string" || !detail) {
    throw new Error("host.emitImage expected detail to be a non-empty string");
  }
  if (!["auto", "low", "high", "original"].includes(detail)) {
    throw new Error(
      'host.emitImage expected detail to be one of "auto", "low", "high", or "original"',
    );
  }
  return detail;
}
...
function normalizeEmitImageUrl(value) {
  if (typeof value !== "string" || !value) {
    throw new Error("host.emitImage expected a non-empty image_url");
  }
  if (!/^data:/i.test(value)) {
    throw new Error("host.emitImage only accepts data URLs");
  }
...
function parseDataUrlMimeType(dataUrl) {
  ...
  if (commaIndex < 0) {
    throw new Error("host.emitImage expected a valid image data URL");
  }
  const mediaType = dataUrl.slice("data:".length, commaIndex).split(";")[0];
  if (!mediaType) {
    throw new Error("host.emitImage expected image data URL to include a MIME type");
  }
  return mediaType;
}
...
function assertEmitImageMimeType(mimeType) {
  ...
  if (!SUPPORTED_EMIT_IMAGE_MIME_TYPES.includes(normalized)) {
    ...
    throw new Error(
      `host.emitImage only supports ${supportedTypes}`,
    );
  }
}
...
function parseContentItems(items) {
  ...
  for (const item of items) {
    if (!isPlainObject(item) || typeof item.type !== "string") {
      throw new Error("host.emitImage received malformed content items");
    }
    ...
    throw new Error(
      `host.emitImage does not support content item type "${item.type}"`,
    );
  }
...
function parseByteImageValue(value) {
  ...
  if (!bytes) {
    throw new Error(
      "host.emitImage expected bytes to be Buffer, Uint8Array, ArrayBuffer, or ArrayBufferView",
    );
  }
...
function parseToolOutput(output) {
  ...
  throw new Error("host.emitImage received an unsupported tool output shape");
}

function normalizeMcpImageData(data, mimeType) {
  if (typeof data !== "string" || !data) {
    throw new Error("host.emitImage expected MCP image data");
  }
  if (/^data:/i.test(data)) {
    return data;
  }
...
function parseMcpImageDetail(meta) {
  ...
  const detail = meta["repl/imageDetail"];
  if (
...
function parseMcpToolResult(result) {
  ...
  if (!isPlainObject(result)) {
    throw new Error("host.emitImage received an unsupported MCP result");
  }
  ...
  if (!isPlainObject(ok) || !Array.isArray(ok.content)) {
    throw new Error("host.emitImage received malformed MCP content");
  }
  ...
  for (const item of ok.content) {
    ...
    throw new Error(
      `host.emitImage does not support MCP content type "${item.type}"`,
    );
  }
...
function requireSingleImage(parsed) {
  if (parsed.textCount > 0) {
    throw new Error("host.emitImage does not accept mixed text and image content");
  }
  if (parsed.images.length !== 1) {
    throw new Error("host.emitImage expected exactly one image");
  }
...
function normalizeEmitImageValue(value) {
  ...
  if (!isPlainObject(value)) {
    throw new Error("host.emitImage received an unsupported value");
  }
...
const host = {
  ...
  tool(toolName, args) {
    ...
    if (typeof toolName !== "string" || !toolName) {
      return Promise.reject(new Error("host.tool expects a tool name string"));
    }
      ...
      await withCapturedConsole(context, async (logs) => {
        ...
        module = new SourceTextModule(source, {
          ...
          initializeImportMeta(meta, mod) {
            ...
            meta.__replInternalMarkCommittedBindings = markCommittedBindings;
            meta.__replInternalMarkPreludeCompleted = markPreludeCompleted;
          },
          importModuleDynamically(specifier, referrer) {
            return importResolved(resolveSpecifier(specifier, referrer?.identifier));
          },
