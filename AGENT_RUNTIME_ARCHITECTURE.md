...
| `mind_app/runtime/turns/subagent_adapter.py` | 组合根执行适配器；后续由 `agent/harness` 消费 | `execute_turn`、`stream_turn` 和静默输出工厂集中在 Turn 适配边界；Subagent 编排不得直接依赖旧 runtime 或具体输出实现，旧 Turn 流退役后删除此适配器 |
