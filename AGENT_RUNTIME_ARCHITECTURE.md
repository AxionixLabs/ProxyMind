...
| `mind_app/runtime/mcp/service_lifecycle.py`、`keepalive.py`、`service_runtime.py` 中的服务上下文值对象 | `infrastructure/services/runtime_owner.py`、`keepalive.py`、`runtime_context.py` | 本地 Helix 服务的启动任务、保活、端口终止和上下文规格属于基础设施；TUI/CLI 只消费服务生命周期入口，不让 runtime 持有平台资源所有权 |
