...
| `mind_app/runtime/turns/root.py` 的前台轮次编排 | `agent/application/turns/foreground.py` + `agent/ports/presentation.py::TurnForegroundLifecyclePort` + terminal adapter | application 只编排执行、完成和清理顺序；终端进度、动画和 worked footer 的具体投影由 `ControllerTurnForegroundLifecycle` 实现，runtime root 只调用 application 用例且不导入前端 |
...
| `agent/application/turns/context.py::TurnContext.animation` | `agent/ports/turns.py::TurnAnimationPort` + `frontends/terminal/animation.py` | 流式执行只消费前台活动状态和等待动画停止端口；Controller/前端负责具体动画实现，执行层不得读取 `frontend.runtime` 或调用 `stop_anim` |
