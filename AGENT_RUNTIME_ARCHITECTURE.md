...
| `mind_app/presentation/terminal/turn_lifecycle.py::run_foreground_turn` | `agent/ports/presentation.py::TurnForegroundLifecyclePort` + `ControllerTurnForegroundLifecycle` | 动画、终端进度、worked footer 和清理属于展示生命周期；runtime root/TUI 只传递生命周期端口，适配器不拥有 Turn/Session 状态 |
...
| `agent/harness/agents/runtime.py::__init__` | `agent/ports/subagents.py::SubagentRuntimeHostPort` + 组合根 | SubagentRuntime 只接收子执行、子轮次 runner、Hook scope 和清理端口；不暴露通用 Turn runtime，Harness 持有 Agent 树、mailbox 和子轮次生命周期 |
| `mind_app/runtime/subagents/execution.py` | 组合根执行适配器；后续由 `agent/harness` 消费 | `execute_turn`、`stream_turn` 和静默输出工厂集中在组合边界；Subagent 编排不得直接依赖旧 runtime 或具体输出实现，纯编排模块迁移后删除此旧路径 |
