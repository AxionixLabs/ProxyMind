...
| `agent/application/turns/context.py::TurnContext.session_context` | `agent/ports/turns.py::TurnSessionContextPort` + `mind_app/runtime/turns/session_context.py` | 流式准备只消费已解析的环境、skills 和动画开关；Controller 配置、环境能力和 skills loader 由组合边界适配，执行层不得直接读取宿主属性 |
