...
| `frontends/tui/session/turn_input.py` | `frontends/tui/session/turn_input.py` + 组合根装配 | `TuiTurnInputControl` 只消费显式注入的 `ProtocolCommandClient`；无客户端时仅保留测试命令 seam，不从 controller 反射发现 `runtime_services`，输入对账和关闭收敛仍归 TUI session 所有 |
