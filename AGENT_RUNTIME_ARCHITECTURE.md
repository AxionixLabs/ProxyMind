...
| `mind_app/history/contracts.py`（已删除） | `agent/ports/transcript.py` | `TranscriptSink` 是 runtime、Hook、执行器和历史 writer 共享的最小写入端口；`TranscriptLifecyclePort` 约束关闭，`TranscriptSessionPort` 约束完整 open/append/close 生命周期，使工厂不依赖实现的隐式方法 |
| `mind_app/history/transcript.py` 中的 `TranscriptEntry`、`TranscriptReplay` | `agent/domain/transcripts.py` | 共享记录值和事件归约器属于纯领域语义，不伪装成持久 Store；文件系统、观测和展示保持在边界外，工具归并策略复用 `agent.domain.tool_policy` |
...
| `mind_app/controller.py` 中的 Session、history、Transcript 与 SessionEnd 事务 | `agent/harness/sessions/root.py` + `agent/ports/conversation.py` + `infrastructure/persistence/conversation_history.py` | Harness 单一拥有根会话状态和结束/归档事务，只依赖历史端口；基础设施组合 SQLite 游标与 Transcript reader；CLI、TUI、MCP 和根 Turn 直接消费根会话端口，Controller 不保留同义方法或持久化属性 |
...
| `mind_app/native_coding/native_coding.py` 中的 Sandbox 构造 | `mind.py::create_workspace_coding`、`infrastructure/workspace/runtime.py` | 唯一进程组合根创建 SandboxClient 和 ProcessSessionManager，并把应用资源根显式交给 `WorkspaceCoding`；工作区实现强制接收已装配会话运行时，工具注册表不得通过空参数隐式创建具体能力 |
| `mind_app/native_coding/js_repl/` | `infrastructure/platform/javascript_repl.py` | Node 内核进程、临时目录、消息桥接和内核重置属于平台执行基座；工作区编码实现只持有内核池生命周期，工具入口、权限和嵌套调用策略由 application 用例负责，并通过端口注入 JSON 调用回调 |
...
| `mind_app/runtime/hooks/tool.py` | `agent/harness/hooks/tool_lifecycle.py` | 前置决定缓存、工具调用/完成去重、PostToolUse 收束和持续命令 Hook 会话属于 Harness 工具生命周期；实现只依赖 Agent 契约和统一 observability，不读取协议客户端、MCP SDK 或历史运行时 |
| `mind_app/runtime/hooks/compact.py`、`session.py`、`turn.py`、`presentation.py` | `agent/harness/hooks/compaction.py`、`session_lifecycle.py`、`turn_lifecycle.py`、`presentation.py` 与 `agent/application/views/builders/hooks.py` | 压缩、SessionEnd、Turn 启停和 Hook 展示通道适配属于 Harness 生命周期；运行快照到中立 view 的纯映射归 application builder。旧 Hook 源包整体删除，Harness 不导入协议客户端、MCP SDK、基础设施或历史包 |
...
| `mind_app/runtime/turns/stream_setup.py`、`stream_finalize.py` 与 `executor.py` 中的 transcript helpers | `agent/adapters/protocol/turn_setup.py`、`agent/harness/execution/turn_finalizer.py`、`agent/application/turns/transcript.py` | wire 环境规范化与输出会话装配归协议 adapter，Turn 终态资源收敛归 Harness，会话/Turn/消息事实写入归 application；finalizer 只消费 `IdleStatusPort` 和 `TranscriptLifecyclePort`，不依赖具体 timer、协议或旧 executor。旧 setup/finalize 同批删除 |
...
| 已删除的 `mind_app/runtime/support/idle_status.py` 与 `infrastructure/platform/idle_status.py` | `agent/harness/execution/idle_status.py` | asyncio 延迟状态计时器是 Turn 执行资源的调度与取消生命周期，不是操作系统能力；Protocol adapter 创建，finalizer 只消费 `IdleStatusPort` |
...
| 已删除的 `mind_app/runtime/turns/executor.py` 中的 `TurnResult`、`TurnOperation` | `agent/ports/turns.py` + `agent/harness/execution/turn_runner.py` | 模型轮次操作只依赖 MCP 会话、事件报告和 TurnExecution；Harness runner 负责会话生命周期、工具过滤和结果收束，旧包不保留 facade |
