...
| `agent/application/turns/context.py::TurnContext.session_state` | `agent/ports/turns.py::TurnSessionStatePort`；迁移期由 Controller 直接实现 | 流式失败、取消上下文和最近 assistant 回复通过会话状态端口写回；执行层不访问 `ConversationState`，Controller 作为当前状态所有者直接履约，不经单调用包装器 |
...
| 已删除的 `mind_app/runtime/turns/root.py::prepare_root_turn` 与三个 Controller facade | `agent/harness/execution/root_runner.py` + `agent/ports/turns.py` | 根轮次会话登记、工作区、权限、Transcript、Hook scope、模型会话和清理均由显式端口提供；Controller 直接实现其已有状态的端口，旧 `runtime/turns` 目录源码清零 |
