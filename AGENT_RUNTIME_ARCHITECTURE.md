...
| `mind_app/runtime/subagents/runtime.py::__init__` | `agent/ports/subagents.py::SubagentRuntimeHostPort` + 组合根 | SubagentRuntime 只接收组合根宿主端口；执行运行时、Hook scope 和清理能力由显式端口提供，删除 Controller、重复执行器参数和宿主类型依赖后，才能迁移剩余运行编排 |
