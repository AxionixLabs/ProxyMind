...
| `frontends/cli`、`mind_app/tui`、`frontends/mcp`、`frontends/subscription` | `frontends/`、`application/`、Protocol Client | CLI、TUI、MCP、Subscription 四类入口均通过 `RuntimeServices` 接收 application；执行命令已冻结并提交统一入口，终态观测和回执优先使用 Run/Canonical Event projection；TUI 作为 Protocol Client adapter，桌面/Web 通过同一 fixture 校验协议投影；剩余 UI/控制器依赖迁出后删除 `mind_app` |
