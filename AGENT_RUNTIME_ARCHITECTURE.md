...
| `agent/harness/agents/runtime.py::__init__` | `agent/ports/subagents.py::SubagentRuntimeHostPort` + 组合根 | SubagentRuntime 只接收组合根宿主端口；执行运行时、Hook scope 和清理能力由显式端口提供，Harness 持有 Agent 树、mailbox 和子轮次生命周期 |
