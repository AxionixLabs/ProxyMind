...
def _tui_package(repository_root: Path) -> Path:
    """返回 TUI 生产包根目录。"""
    return repository_root / "frontends" / "tui"


def test_rendering_does_not_import_core(repository_root: Path) -> None:
    """保证 rendering 层不会重新依赖旧 core 实现。"""
    tui_package = _tui_package(repository_root)
    violations: list[str] = []
...
def test_runtime_state_does_not_import_screen_or_runtime(
...
def test_resume_runtime_coordinator_uses_narrow_ports(
    repository_root: Path,
) -> None:
...
def test_process_feature_uses_capability_port(repository_root: Path) -> None:
...
def test_foreground_barrier_uses_capability_port(repository_root: Path) -> None:
...
def test_turn_execution_uses_capability_port(repository_root: Path) -> None:
...
def test_turn_input_uses_capability_port(repository_root: Path) -> None:
...
def test_read_only_features_use_capability_ports(repository_root: Path) -> None:
