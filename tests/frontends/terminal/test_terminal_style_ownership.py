...
def test_regular_terminal_components_do_not_own_concrete_colors(
    repository_root: Path,
) -> None:
    style_roots = (
        repository_root / "frontends" / "cli",
        repository_root / "frontends" / "terminal",
        repository_root / "frontends" / "tui",
    )
    rich_color_owners = frozenset({
        repository_root / "frontends" / "terminal" / "highlighting.py",
        repository_root / "frontends" / "terminal" / "renderers" / "patch.py",
        repository_root / "frontends" / "terminal" / "semantic_styles.py",
    })
    violations: list[str] = []
    for root in style_roots:
        for path in root.rglob("*.py"):
            if path in rich_color_owners:
                continue
