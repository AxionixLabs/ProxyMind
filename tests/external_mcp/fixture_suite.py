...
async def serve(directory: Path, *, launch_client: bool = False) -> None:
    ...
    async with contextlib.AsyncExitStack() as stack:
        http = await stack.enter_async_context(remote_fixture(FixtureSpec(directory, "H", "streamable_http")))
        sse = await stack.enter_async_context(remote_fixture(FixtureSpec(directory, "S", "sse")))
        path = write_config(directory, (http, sse))
        print(f"Fixture config: {path}", flush=True)
        if launch_client:
            process = await asyncio.create_subprocess_exec(*client_arguments(path), cwd=directory)
            try:
                await process.wait()
            finally:
...
def main() -> None:
    ...
    try:
        asyncio.run(serve(values.directory.resolve(), launch_client=values.client))
