...
def test_view_image_returns_image_attachment(tmp_path: Path) -> None:
    """可访问图片以 data URL 附件回传。"""
    image = tmp_path / "pixel.png"
    image.write_bytes(PNG_BYTES)
    tool = tool_by_name(view_image_tools(NativeCoding(root=tmp_path)), VIEW_IMAGE_TOOL)
...
def test_view_image_is_in_default_client_registry(tmp_path: Path) -> None:
    """默认客户端工具注册表会对外上报图片查看工具。"""
    registry = default_registry(NativeCoding(root=tmp_path))

    assert registry.has_tool(VIEW_IMAGE_TOOL) is True


def test_view_image_reads_absolute_path(tmp_path: Path) -> None:
    """绝对路径图片可在系统允许读取时回传。"""
    image = tmp_path / "outside.png"
    image.write_bytes(PNG_BYTES)
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    tool = tool_by_name(view_image_tools(NativeCoding(root=workspace)), VIEW_IMAGE_TOOL)
...
def test_view_image_reads_relative_path_outside_workspace(tmp_path: Path) -> None:
...
def test_view_image_rejects_unsupported_content(tmp_path: Path) -> None:
    """非图片内容不会仅依据文件扩展名接受。"""
...
def test_view_image_rejects_oversized_file(tmp_path: Path) -> None:
    """超过上限的图片不会被编码进请求载荷。"""
