...
def _sandbox_layout_or_skip(repository_root: Path) -> ApplicationLayout:
...
@pytest.mark.anyio
async def test_sandboxed_exec_command_uses_sidecar_pty(
    tmp_path: Path,
    ...
    coding = create_workspace_coding(
        root=tmp_path,
        application_layout=_sandbox_layout_or_skip(repository_root),
        network_access="enabled",
    )
    try:
        started = await coding.exec_command(
...
@pytest.mark.anyio
async def test_sandboxed_pty_delivers_terminal_eof(
    tmp_path: Path,
    repository_root: Path,
) -> None:
...
@pytest.mark.anyio
async def test_sandboxed_pty_interrupt_keeps_process_interactive(
    tmp_path: Path,
    repository_root: Path,
) -> None:
