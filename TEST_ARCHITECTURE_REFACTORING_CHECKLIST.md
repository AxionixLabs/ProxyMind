...
并通过全部合并验证，已推送为 `fd40f25d`。阶段 5 CI 分层门禁已推送为 `47b3b6ca`。阶段 6 风险
覆盖和重复用例治理已完成本地复核，推送后进入阶段 7 长期守护。继续改造时仍按真实层级确定主要
生命周期，不能按文件名前缀机械归类：
...
| 当前全树收集         | `4228`；阶段 6 新增 5 条质量审计，另含恢复后已有的 1 条 Runtime 测试，无收集错误                           |
...
| 阶段 6 静态收口      | 完整架构审计 `137 passed`；全树收集、Runtime P0、YAML、compileall 与差异检查通过                        |
...
- [x] 夜间全量按生产包统计 branch coverage；风险维度由 node id 和 marker 证据判断，不设置总百分比目标。
- [x] 定向 mutation 候选限定为协议事件 parser、canonical item reducer、审批规则和本地副作用账本；
  branch gap 审阅前不进入默认依赖或 CI。
