...
- 测试目录表达稳定的责任所有者，目录不表达速度、测试等级或执行频率；
...
- 快速验证、Runtime 风险验证、平台验收和全量回归尚未形成明确的本地执行矩阵；
...
| adapter 集成  | HTTP、SSE、文件、sidecar、终端 adapter | 本地 fake server/process   | 验证边界转换和故障分类   | 相关修改和收口   |
| 客户端验收    | 完整 Turn、TUI、PTY、恢复流程          | 只替代真实外部系统         | 验证用户可观察行为       | 阶段或发布收口   |
...
候选 marker 只有在维护者实际使用的选择命令和责任 owner 同时落地时才可新增：
...
| 层级       | 选择                      | 执行时机               | 失败处理                 |
|------------|---------------------------|------------------------|--------------------------|
| 定向验证   | 受影响责任目录或测试文件  | 本地每次修改           | 立即修复                 |
| 快速全量   | 排除真实 PTY 和明确 slow  | 阶段和发布收口         | 停止收口并修复           |
| Runtime P0 | `-m runtime_p0`           | Runtime 修改和发布前   | 停止收口并修复           |
| 平台验收   | PTY、sidecar、平台 marker | 平台边界修改和发布前   | 对应平台复核后继续       |
| 完整回归   | 全测试树                  | 测试架构阶段和发布收口 | 建立责任人并修复         |
| 稳定性回归 | 固定 seed 矩阵和有限重复  | 风险审阅和发布收口     | 输出 seed 与首个失败场景 |
...
并通过全部合并验证，已推送为 `fd40f25d`。阶段 5 的本地执行矩阵已完成；曾提交的测试 CI 配置
`47b3b6ca` 已按维护决定移除。阶段 6 风险覆盖和重复用例治理、阶段 7 长期守护均已完成本地
复核。后续维护仍按真实层级确定主要生命周期，不能按文件名前缀机械归类：
...
| macOS sidecar 定向集 | 当前 Windows 环境 `11 skipped`，发布前需在 macOS 实机手动验证                                         |
...
| 当前全树收集         | `4236`；阶段 7 质量审计共 6 条，无收集错误                                                             |
...
| 阶段 5 本地矩阵      | Windows 平台非 PTY 为 `61 passed, 12 skipped, 4 deselected`；仓库不再配置自动化测试 CI                 |
| 阶段 5 平台验收      | 全局 PTY 为 `132 passed, 3 skipped`；其他平台结果由维护者在对应实机手动复核                            |
| 阶段 6 质量审计      | 环境、随机、重试隔离、场景真实等待和直接公网入口共 `5 passed`                                         |
| 阶段 6 覆盖率试点    | 四个候选责任面 `110 passed`，branch coverage 合计 75%，仅用于发现分支缺口                              |
| 阶段 6 进程成本      | 句柄重复释放仍逐次断言，连续周期由 100 收敛为 8，Windows 耗时由约 29 秒降为 2.3 秒                      |
| 阶段 6 确定性修复    | 布局测试改用同步 completion 契约，专题 `23 passed`；非 PTY 全量 `4081 passed, 12 skipped, 135 deselected` |
| 阶段 6 静态收口      | 完整架构审计 `137 passed`；全树收集、Runtime P0、compileall 与差异检查通过                              |
| 阶段 7 最终复核      | 本地完整回归 `4220 passed, 15 skipped`；最终架构审计 `138 passed`；compileall 与差异检查通过            |
...
- [x] 五个现有 marker 均已记录收集清单和数量，并保留稳定选择命令。
- [x] 已运行完整测试集，记录通过、跳过、耗时和最慢测试，基线失败与改造失败分别处置。
- [x] Windows 可运行集合已实测；Linux、macOS 保留稳定手动命令和能力边界，平台缺失不记为通过。
- [x] 迁移批次均记录原路径、目标 owner、路径依赖、稳定文档引用和迁移前后 node id。
...
- [x] 定义排除真实 PTY 的快速全量命令；当前没有经定义的 slow marker。
- [x] 定义独立 `runtime_p0` 命令，其与快速全量重叠但仍保留为可单独复核的风险集合。
- [x] 平台 adapter 和 PTY 集合由维护者在 Windows、Linux、macOS 对应实机手动执行。
- [x] 完整回归输出最慢测试和失败 seed，由测试架构阶段或发布收口主动执行。
- [x] 发布前手动运行完整测试、架构审计、compileall 和 `git diff --check`。
- [x] 只有稳定选择命令和责任 owner 同时存在时才注册 `slow`、`external_process`、平台或 `serial` marker；本阶段
  没有新增 marker。
- [x] 审计固定端口、进程环境、单例缓存和共享目录后，暂不启用 pytest-xdist；并行隔离尚未证明。
- [x] 按维护决定删除 `.github/workflows/tests.yml`，仓库不再配置自动化测试 CI。

完成门槛：本地分层命令能提供确定性反馈；平台失败不会被其他平台的 skip 掩盖；稳定性失败可以
由 node id、seed 和事实 trace 重放。

### 阶段 6：风险覆盖和重复用例治理

- [x] 为核心风险矩阵逐项标注现有快速、故障和验收证据，稳定 node id 已进入 `tests/README.md`。
- [x] 本地风险审阅按生产包统计 branch coverage；风险维度由 node id 和 marker 证据判断，不设置总百分比目标。
- [x] 定向 mutation 候选限定为协议事件 parser、canonical item reducer、审批规则和本地副作用账本；
  branch gap 审阅前不进入默认依赖或日常验证。
- [x] AST 盘点得到 46 个仅含 mock 断言的候选；复核后其断言对象均为正式 port、终端写入或禁止副作用，
  没有为满足形式而增加重复状态断言。
- [x] 大型字典没有跨文件完全重复；四组同键载荷属于 schema、adapter、fake 的独立严格边界向量，
  共享反而会掩盖未知字段或错误 owner，因而不提取通用 builder。
- [x] 日志断言只存在于 MCP 第三方日志降噪与凭据清理边界；异常文本用于输入错误定位，业务状态继续
  由结构化事件、具名异常类型和正式投影断言。
- [x] 真实等待集中在 TUI、PTY、MCP 进程边界；100 次进程创建缩减为 8 次逐次释放断言，场景与 Fake
  的零真实等待由架构审计守护。固定端口没有用于监听，本地 server 绑定动态端口并显式关闭。
- [x] 当前没有 flaky、rerun 或 quarantine；架构审计禁止引入这些机制和无 seed 的模块级随机调用。
...
- [x] 测试评审模板已收敛到唯一长期说明，覆盖所有者、主要不变量、验证深度、关键失败路径、
  替身边界、确定性、资源关闭、最小命令和重复处置。
- [x] 季度命令检查最慢集合、全树与 marker 收集、skip reason、架构边界和本地 flaky 记录。
- [x] marker 注册、实际使用和稳定选择命令由架构审计保持一致；目录或 marker 变化必须在同一提交
  同步 `tests/README.md` 和稳定架构文档，不维护迁移流水账。
...
- 修改根架构审计、包边界、pytest 配置或发布收口规则时必须运行完整架构审计；
...
- [x] 所有长期有效规则和命令已收敛到 `tests/README.md`，本文仅剩可删除的完成记录；
- [x] `tests/` 根目录只保留约定的稳定入口、配置和说明文件；
- [x] 每个自动化测试存在唯一、可解释的责任所有者；
- [x] backend 独立服务实现不由客户端测试树直接导入或验证；
- [x] 当前五个风险 marker 有稳定选择命令，新增 marker 均有明确维护责任；
- [x] 不存在依赖测试文件目录深度的仓库根解析；
- [x] 不存在无说明的超大多职责测试模块；
- [x] Turn、审批、工具、replay、终态、资源关闭和 TUI 原子帧均有快速确定性门禁；
- [x] Fake Server、场景模型和 PTY harness 各自拥有明确契约与关闭生命周期，不存在通用 support 包；
- [x] 自动化测试不访问公网、不依赖真实用户配置、不依赖执行顺序；
- [x] 定向、快速全量、Runtime P0、平台验收、完整回归和发布收口各有清晰职责；
- [x] 全量测试、架构审计、compileall 和差异检查通过；
- [x] `AGENTS.md`、架构文档、PTY 文档和测试说明中的稳定路径保持有效。
