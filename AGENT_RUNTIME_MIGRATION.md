...
1. **application/harness/stores 的公开 API 审计**：列出真实消费者，收窄包级导出；删除
   无消费者的导出和重复值对象。必须保持旧路径不存在，不能新增兼容 facade。
2. **组合根依赖清理**：让 `agent/composition.py` 成为具体实现的唯一装配点；消除
   application、harness、stores 对配置解析、环境探测和具体网络客户端的隐式导入。
3. **历史副作用适配器退役**：按完整权限进程、Helix、Sandbox sidecar、根轮次 executor
   的真实消费者逐项接入 capability/application，完成后删除旧调用路径和 `engine` 模块。
4. **入口与数据迁移**：将 `mind_core` 的配置、权限、hooks、skills 按职责落位；在
