...
def bind(mcp: FastMCP, idle: Idle) -> None:
    ...
    async def ffmpeg_extract_frames(
        ...
        """Class: ffmpeg; Action: 拆帧; Args: input_video(str)=输入视频, output_dir(str)=输出目录, pattern(str)=文件名模板, fps(float|None)=抽帧帧率, image_format(jpg|png|webp)=图片格式, start_sec(float|None)=起始秒, duration_sec(float|None)=时长秒, scale_w/int|None=缩放宽, scale_h/int|None=缩放高, overwrite(bool)=覆盖输出; Use: 将视频导出为图片序列，支持抽帧/截取/缩放; Return: CallToolResult(text + structuredContent); Notes: 输入路径或输出目录不可用会失败。"""
    ...
    async def ffmpeg_remux_video(
        ...
        *,
        ...
        """Class: ffmpeg; Action: 视频换封装(re_mux); Args: input_video(str)=输入视频路径, output_video(str)=输出视频路径(生成新文件), overwrite(bool=True)=覆盖输出; Use: 仅换容器/封装不重编码（最快，mkv<->mp4 等）; Return: CallToolResult(text + structuredContent); Notes: 流/容器不兼容或输出不可写会失败。"""
        await Requires.connect_ffmpeg()
    ...
    async def ffmpeg_extract_audio(
        ...
        *,
        ...
        """Class: ffmpeg; Action: 抽取音轨; Args: input_video(str)=输入媒体路径(常为视频), output_audio(str)=输出音频路径(生成新文件), audio_codec(str|None)=音频编码器(None=自选), overwrite(bool=True)=覆盖输出; Use: 从媒体文件输出音频文件（-vn，不输出视频）; Return: CallToolResult(text + structuredContent); Notes: 格式由 output_audio 扩展名决定。"""
        await Requires.connect_ffmpeg()
    ...
    async def ffmpeg_replace_audio(
        ...
        *,
        ...
        """Class: ffmpeg; Action: 替换视频音轨; Args: input_video(str)=输入视频路径, input_audio(str)=输入音频路径, output_video(str)=输出视频路径(生成新文件), keep_video(bool=True)=视频copy不重编码, audio_codec(str='aac')=输出音频编码器, overwrite(bool=True)=覆盖输出; Use: 保留画面替换为新音频（可做配音/换BGM）; Return: CallToolResult(text + structuredContent); Notes: 默认 -shortest 以较短轨道为准。"""
        await Requires.connect_ffmpeg()
    ...
    async def ffmpeg_video_snapshot(
        ...
        *,
        ...
        """Class: ffmpeg; Action: 视频取帧截图; Args: input_video(str)=输入视频路径, output_image(str)=输出图片路径(生成新文件), at_sec(float=0)=截图时间点(秒), overwrite(bool=True)=覆盖输出; Use: 从视频指定时间点导出单帧图片（封面/缩略图）; Return: CallToolResult(text + structuredContent); Notes: Action 为“视频取帧截图”以避免与 adb screenshot 混淆。"""
        await Requires.connect_ffmpeg()
    ...
    async def ffmpeg_mute_video(
        ...
        *,
        ...
        """Class: ffmpeg; Action: 视频去音轨(mute/remove-audio); Args: input_video(str)=输入视频路径, output_video(str)=输出视频路径, overwrite(bool=True)=覆盖输出; Use: 输出静音视频（视频流 copy 不重编码，速度最快）; Return: CallToolResult(text + structuredContent); Notes: 仅移除音轨；不会改变画面质量/编码。"""
        await Requires.connect_ffmpeg()
