...
def bind(mcp: FastMCP, idle: Idle, ctx: AppContext) -> None:
    ...
    @mcp.tool(
        description=(
            "在工作区内执行单条本地 shell 命令。"
            " command 是命令字符串，可包含 cwd 和 timeout_sec。"
    ...
    async def shell_command(
        ...
        cwd: ShellCwdArg = ".",
        timeout_sec: ShellTimeoutSecArg = 60,
    ...
    async def shell_calls(
        ...
        job_id = await idle.job_begin("native_coding.shell_calls", args=args)
        try:
            raw = await ctx.native_coding.shell_calls(**args)
        finally:
            await idle.job_final(job_id)

        return build_tool_result(tool="shell_calls", args=args, raw=raw, target=ctx.native_coding.agent_id)
