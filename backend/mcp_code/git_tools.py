...
class GitTools(NativeCodingComponent):
    ...
    async def _git_diff_stats(
        ...
        *,
        ...
        rel_path = self._rel(self._resolve(path)) if path else None

        unstaged_numstat = await self._git(
            ["diff", "-M", "--numstat", "--", *([rel_path] if rel_path else [])],
            output_limit=output_limit
        )
        staged_numstat = await self._git(
            ["diff", "--cached", "-M", "--numstat", "--", *([rel_path] if rel_path else [])],
            output_limit=output_limit
        )
        unstaged_name_status = await self._git(
            ["diff", "-M", "--name-status", "--", *([rel_path] if rel_path else [])],
            output_limit=output_limit
        )
        staged_name_status = await self._git(
            ["diff", "--cached", "-M", "--name-status", "--", *([rel_path] if rel_path else [])],
            output_limit=output_limit
        )
        unstaged_hunks = await self._git(
            ["diff", "-M", "--unified=0", "--", *([rel_path] if rel_path else [])],
            output_limit=output_limit
        )
        staged_hunks = await self._git(
        ...
        untracked = await self._untracked_diff_section(path=rel_path)
    ...
    def _parse_diff_hunks(
        ...
        for line in str(diff or "").splitlines():
            if line.startswith("diff --git "):
                ...
                current = files.setdefault(path, {"path": path, "hunk_count": 0, "hunks": []})
                hunk    = None
    ...
    async def _untracked_diff_section(
        ...
        *,
        ...
        result = await self._git(cmd, output_limit=self.max_output_chars)
    ...
    def _expand_untracked_path(
        ...
        files: list[Path] = []
        for item in self._walk(target, recursive=True):
            if item.is_file() and not self._is_excluded(item):
                files.append(item)
                if len(files) >= 200:
                    break
        return files
    ...
    def _untracked_file_stat(
        ...
        binary = not self._looks_text(target)
        ...
        if not binary:
            ...
            added_lines = len(self._decode(raw).splitlines())
