...
def helix_home() -> Path:
    ...
    return Path(os.environ.get(HX_HOME_ENV) or application_home() / "helix").expanduser()
...
def reports_dir() -> Path:
    """返回应用报告输出目录。"""
    return application_home() / "reports"
...
def history_dir() -> Path:
    """返回对话历史本地目录。"""
    return application_home() / "history"
...
def ensure_application_home() -> Path:
    """确保应用的用户级目录及常用子路径可写。"""
    try:
        root = ensure_writable_dir(application_home())

        ensure_writable_dir(reports_dir())
        ensure_writable_dir(sessions_dir())
        ensure_writable_dir(history_dir())

        return root

    except OSError as exc:
        raise AppError(
            f"Home is not writable: {application_home()} "
