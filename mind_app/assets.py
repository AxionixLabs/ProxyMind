...
class EntryUpgradeProgress(object):
    ...
    def __init__(self, anim_manager: AsyncAnimManager) -> None:
        """绑定入口级动画管理器。"""
    ...
    async def start(self, state: dict) -> None:
        ...
        await self.anim_manager.start(
            lambda stop_event: Design.download_animation(state, stop_event)
        )
...
async def ensure_asset(
    *,
    ...
    up: Upgrade = Upgrade()
    await up.upgrade_app(supports, progress=EntryUpgradeProgress(anim_manager))
