...
def _normalize_source(item: typing.Any) -> CodeSourcePayload:
    ...
    if not entry:
        raise AppError("Code source entry is empty")
...
async def _resolve_url(src: CodeSourcePayload) -> CodeSourceResolved:
    ...
    try:
        ...
        raise AppError(f"Code source URL timeout: {url}") from exc
    except httpx.HTTPStatusError as exc:
        raise AppError(f"Code source URL failed: {url} status={exc.response.status_code}") from exc
    except httpx.HTTPError as exc:
        raise AppError(f"Code source URL request failed: {url}") from exc

    raw = response.text
    raw_bytes = len(raw.encode(const.CHARSET, errors="replace"))
    if raw_bytes > max_bytes:
        raise AppError(f"Code source URL too large: {url} bytes={raw_bytes}")
...
async def resolve_code_sources(code: list[typing.Any]) -> list[CodeSourceResolved]:
    ...
    if not code:
        raise AppError("Code list is empty")
    ...
    for src in normalized:
        ...
        if src.kind == "stdin":
            ...
            if not stdin_cache.strip():
                raise AppError("STDIN code source is empty")
