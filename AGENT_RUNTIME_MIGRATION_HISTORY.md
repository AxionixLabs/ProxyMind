# Agent Harness 迁移历史归档

> 本文件是迁移计划的历史交接归档，不是当前状态权威。当前阶段、准入条件、
> 出口条件、未决风险和下一步动作只以
> [AGENT_RUNTIME_MIGRATION.md](AGENT_RUNTIME_MIGRATION.md) 为准。
>
> 归档内容保留了 2026-08-31 之前的阶段明细、已完成切片证据和变更记录，供
> 审计、回溯和交接时查阅。历史状态描述不应覆盖主计划中的最新状态。
...
- [x] `LocalEffectJournal` 已从 `mind_app/runtime/durable_effects.py` 迁入
  `agent/stores/effects/journal.py`，旧文件已删除且没有转发 facade；效果端口要求
...
- [x] `agent.capabilities` 只依赖 `agent` 内部边界；迁移期正式协议传输依赖已集中到
  `agent.adapters.protocol.client`，两者均禁止导入 `mind_app`、`mind_core`、`engine`、
  `server` 或 `backend`。
...
游标提交已从 `Mind`/控制器迁入 `agent.adapters.protocol.client`。Canonical Item
...
状态：首轮职责化重组已完成，历史包迁移进行中（2026-08-31）

阶段 4 出口已经证明当前 `agent/` 分层可以承载 Harness 主线；首轮职责化重组已按
本节约束完成，后续只在独立生命周期、一致性或扩展边界成立时继续拆分。本节同时冻结
...
| `agent/ports` | 能力、Session 生命周期和持久化的最小 Protocol/ABC 契约 | 保持；删除无消费者的过渡端口 | 每个端口都有生产实现或明确测试替身 |
...
| `agent/adapters/protocol/client.py`、`items.py` | `mind.chat` wire 翻译、恢复游标和 Canonical Item 投影 | 冻结公共边界；作为 `protocol/` SDK 的 Harness 适配层 | 跨前端兼容测试、版本所有权和旧请求模块删除证据齐备 |
...
阶段 5 后续不再按历史文件一对一搬迁。现有顶层边界已经稳定，当前结构债务集中在
`application/`、`harness/` 和 `stores/` 内部的平铺模块，以及过大的包级导出面。本次
重组先冻结以下职责树，再按完整生产用例逐波切换；每一波都必须删除旧平铺路径，不能
通过同名转发模块掩盖未完成的边界迁移。

| 波次 | 目标目录 | 所有权 | 删除条件 |
| --- | --- | --- | --- |
| A | `agent/application/agents`、`turns`、`hooks`、`config` | 子 Agent 值对象、Turn 用例与结果、Hook 契约、启动设置分别归类；`services.py` 保持组合无关的应用入口 | 所有生产/测试消费者切换，`application` 不再直接依赖旧平铺路径，公开导出收窄且定向回归通过 |
| B | `agent/harness/agents`、`execution`、`sessions` | Agent 树状态、Run/Subagent 执行和 Session 单写者按生命周期分组 | 控制、接管、取消、并发和关闭用例通过，旧文件物理删除，Harness 导入图无循环 |
| C | `agent/stores/agents`、`runs`、`effects`、`approvals` | 图/mailbox、Run 事实、Effect 账本、审批授权按持久化事实分组 | 每组事务边界和恢复测试独立通过，所有消费者切换，`stores` 包级 API 只保留明确的存储入口 |
| D | `agent/adapters/protocol`、`agents` | Protocol Client/Item 投影和子 Agent 外部交互适配分离 | 线上协议 fixture、Canonical Item、工具/审批和 Subagent 回归通过，旧平铺 adapter 删除 |
| E | 组合根与公开 API 收口 | `composition.py` 负责唯一装配；application/harness 子包不再承担跨层自动发现 | 导入图只保留组合根到具体实现的依赖，所有入口使用具名模块，旧 facade 和历史包删除条件满足 |

重组准入规则：新目录必须在同一变更中承载真实调用者；移动后必须更新导入图、架构
守卫、定向行为测试和 `compileall`；如果一个目录只有一个无独立生命周期的文件，保持
在其父目录，避免为了形式整齐制造空壳层。`agent/protocol`、`agent/domain`、
`agent/ports`、`agent/capabilities` 当前已经满足职责聚合，不在本轮机械拆分。
...
1. **元数据边界拆分（已完成首个切片）**：将版本、编码和展示常量从协议传输中
   ...
   不混入该包。
2. **`agent/` 职责化重组（当前波次）**：按上表 A-D 依次建立真实子包，切换并删除
   旧平铺模块；迁移过程中先收敛内部导入，再评估 `application/__init__.py` 和
   `stores/__init__.py` 的最小公开面，不以兼容 facade 延长旧路径生命周期。
3. **公共边界冻结**：核对 `agent.protocol`、重组后的 `agent.application`、
   `agent.ports` 的导出面和依赖方向；禁止新增只转发一次调用的入口。
4. **Protocol SDK 迁移（已完成结构切片）**：顶层 `protocol/` 作为正式 `mind.chat`
   Python wire SDK 的最终所有者，按 `schema`、`transport`、`client` 分层负责请求/响应
   schema、HTTP、认证、事件解析和 endpoint 错误；`agent.adapters.protocol.client` 只
   负责冻结请求映射、Session 游标、Canonical Item 投影和 `ProtocolCommandError` 归一化。
   后续只补齐版本兼容和跨前端 fixture，不恢复旧扁平路径。
5. **历史副作用适配器退役**：按完整权限进程、Helix、Sandbox sidecar、根轮次
   executor 的实际消费者逐项处理；每项必须先由 capability/application 接管，再删除
   旧调用路径和 `engine` 模块。
6. **配置与入口迁移**：将 `mind_core` 的配置、权限、hooks、skills 按职责拆入
   `agent.domain`、`agent.application` 和 `infrastructure/config`；CLI、TUI、MCP、
   Subscription 的完整用例迁入 `frontends/`，不以批量目录改名代替边界迁移。
7. **历史包删除和收口**：按导入图逐批删除 `mind_app`、`mind_core`、`mind_nova`、
...
- [x] 将 `mind_core/agent_config.py` 与 `mind_core/feature_config.py` 合并迁入
  `agent/application/config/settings.py`，由 application 公开入口提供 Agent 并发设置、能力
...
- [x] 将 `mind_app/approval/permission_grants.py` 和 `ledger.py` 重组为
  `agent/stores/approvals/permissions.py` 与 `ledger.py`，由 stores 统一持有
...
- [x] 将环境能力调用、返回值边界校验和 `CapabilityError` 收敛从
  ...
  `agent/application/turns/environment.py`；应用核心只依赖环境能力端口和协议值类型，失败
...
- [x] 将纯 Hook 生命周期快照、决定、输出条目和工具结果值对象从
  `mind_app/runtime/hooks/models.py` 重组到 `agent/application/hooks/models.py`；新模块只
...
- [x] 将 Hook 进程 stdin/stdout schema、事件输入构建和输出校验从
  `mind_app/runtime/hooks/protocol.py` 重组到 `agent/application/hooks/protocol.py`；该模块
...
- [x] 将 Hook 管理目录、不可变状态快照和内容冲突错误从
  `mind_app/runtime/hooks/catalog.py` 重组到 `agent/application/hooks/catalog.py`，切换
...
- [x] 将 Hook 输出 schema 后的语义校验、决定归一化和业务阻断结果从
  `mind_app/runtime/hooks/effects.py` 重组到 `agent/application/hooks/output.py`；该模块
  不持有外部效果或执行器副作用。
- [x] 将生命周期事件规格目录和目录一致性校验从
  `mind_app/runtime/hooks/events.py` 重组到 `agent/application/hooks/events.py`，只通过同层
...
- [x] 将后置 Hook 的工具结果替换、反馈、阻断和追加上下文投影从
  `mind_app/runtime/hooks/results.py` 重组到 `agent/application/hooks/result.py`；该模块
...
- [x] 将单次模型运行的不可变 `RunResult` 从
  `mind_app/runtime/turns/result.py` 重组到 `agent/application/turns/run_result.py`，将流式终态
  优先级、协议终态归并和结果构建从 `stream_outcome.py` 重组到
  `agent/application/turns/stream_outcome.py`；application 公开入口统一提供两类结果契约，前端、
  Subagent、MCP 和 Subscription 不再直接依赖 runtime turns。
- [x] 将远端 `cid/sid` 到本地 Session 身份的确定性派生从
  `mind_app/runtime/support/session_identity.py` 重组到 `agent/application/config/session_identity.py`；
...
- [x] 将 `AgentMailboxEvent`、`AgentMailboxSnapshot`、`AgentMailboxStore` 及其有界日志、消费
  ...
  `agent/stores/agents/mailbox.py`；mailbox 状态由 stores 统一持有，runtime/subagents 只负责
  控制和调度。
...
- [x] 将 `AgentThreadContext`、`AgentTurnContext` 从 `mind_app/runtime/subagents/thread.py` 重组到
  `agent/application/agents/thread.py`；application 契约只依赖 domain、协议 schema 和 application
  执行/设置类型，不依赖 runtime、历史或具体基础设施。
- [x] 将 `ForkContextSnapshot`、`ForkTurns` 和 `normalize_fork_turns` 重组到
  `agent/application/agents/fork_context.py`；`mind_app/runtime/subagents/context.py` 仅保留 transcript
...
- [x] 将 `AgentGraphRecord`、`AgentGraphCheckpoint`、`AgentGraphStore` 和
  ...
  `agent/stores/agents/graph.py`；control 仅保留可变调度协调器，SQLite schema、载荷校验和单写者
...
- [x] 将 `SteeringMessageDelivery` 提升到 `agent/adapters/agents/messages.py`，集中承担
...
- [x] 将 `HookExecutionContext` 与权限模式映射从 `mind_app/runtime/hooks/scope.py` 重组到
  `agent/application/hooks/context.py`；上下文只依赖 Turn、domain Hook 事件名和已定义的 Hook schema，
...
- [x] 将不可变 `TurnExecution` 从 `mind_app/runtime/turns/executor.py` 重组到
  `agent/application/turns/execution.py`；模型身份、输入载荷、metadata 和追加上下文的冻结逻辑由
  application 统一持有。
...
- [x] 将 `SubagentHookEvents` 从 `mind_app/runtime/hooks/subagent.py` 重组到
  `agent/application/hooks/subagent.py`；生命周期事件聚合、续跑决定和上下文上限只依赖
...
- [x] 将 `SubagentRunner` 从 `mind_app/runtime/subagents/runner.py` 重组到
  `agent/harness/execution/subagent_runner.py`；续跑构造 `create_continuation_execution` 同步归入
...
- [x] 将 `StreamSubagentExecutor` 从 `mind_app/runtime/subagents/executor.py` 重组为
  `agent/adapters/agents/execution.py` 的 `StreamSubagentExecution`；adapter 只依赖
...
- [x] 将完整 `AgentControl` 可变树状态机从 `mind_app/runtime/subagents/control.py` 重组到
  `agent/harness/agents/control.py`；Harness 现在拥有 Agent 生命周期、队列、mailbox 协调、
...
- [x] 将 `AgentActiveTurn`、`AgentMessageDispatch` 和活动投递等待状态从
  `mind_app/runtime/subagents/delivery.py` 重组到 `agent/harness/agents/delivery.py`；状态机只
...
- [x] 将 SubagentRuntime 内部的活动 Turn 注册、同 Agent 旧轮次关闭、并发注销和根会话清理
  逻辑收敛到 `agent/harness/agents/delivery.py` 的 `AgentDeliveryRegistry`；runtime 不再持有
  活动投递字典或锁。
...
- [x] 将 `AgentSnapshot`、`AgentWaitResult`、`AgentMailboxWaitResult` 从
  `agent/harness/agents/control.py` 重组到 `agent/application/agents/views.py`；只读状态、等待结果
...
- [x] 将 SubagentRuntime 持有的根会话 control 字典、并发锁和 shutdown 生命周期重组到
  `agent/harness/agents/registry.py` 的 `AgentControlRegistry`；恢复/创建通过显式 control factory 注入。
...
- [x] 将继承上下文条目、轮次分组、范围选择、JSON 渲染和字符预算算法重组到
  `agent/application/agents/fork_context.py`；新增 `ForkContextEntry`，application 不再依赖历史存储类型。
...
### 已完成切片：Subagent 提交执行协调归位

状态：已完成（2026-08-31）

- [x] 将 SubagentRuntime 的 `_execute_submission` 重组到 `agent/harness/execution/subagent_submission.py`，由
  `SubagentSubmissionExecutor` 统一 mailbox claim/release、Turn 执行上下文、活动投递和结果确认。
- [x] Hook scope、permission grants、SubagentRunner、执行 adapter 和活动投递注册表全部显式注入；runtime 删除重复的失败异常与提交执行实现。
...
- [x] 将不可变 `AgentMessageDispatch` 结果从 `agent/harness/agents/delivery.py` 重组到
  `agent/application/agents/messages.py`，Harness delivery 仅保留活动 Turn 和注册表状态。
...
- [x] 将不可变 `CompactResult` 从 `mind_app/runtime/conversation.py` 提升到
  `agent/application/turns/compact_result.py`，通过 application 公开入口提供稳定的压缩结果契约；
  runtime 不再定义结果值对象。
...
- [x] 将 `AgentContext`、`TurnContext` 和 `ToolInvocation` 从
  `mind_app/runtime/execution.py` 重组到 `agent/application/turns/context.py`，并通过
...
- [x] 将 application 按 `agents`、`turns`、`hooks`、`config` 分组；Agent 上下文、Turn
  结果、Hook 契约和启动设置不再在 application 根目录平铺。application 子包之间使用
  职责模块显式导入，内部模块不再通过包级 facade 互相加载。
- [x] 将 harness 按 `agents`、`execution`、`sessions` 分组；AgentControl、活动投递、
  根注册表、Run/Subagent 执行和 Session 单写者的生命周期归属与代码结构一致，旧平铺
  文件已物理删除。
- [x] 将 stores 按 `agents`、`runs`、`effects`、`approvals` 分组，并将 adapters 按
  `protocol`、`agents` 分组；Run schema/records、Agent graph/mailbox、Effect journal、
  审批授权以及 Protocol/子 Agent 适配边界不再混排。
- [x] 所有生产与测试消费者切换到新路径；架构守卫更新为职责化路径并保留旧路径删除
  断言。`AgentMessageDispatch` 使用最小 `AgentMessageEvent` 端口形状，不再让
  application 类型检查依赖具体 mailbox store。
- [x] skills 解析从 SubagentRuntime 移到组合根 `open_skills_provider`，runtime 只接收
  注入的 `SkillsProvider`；Controller 不再读取配置或基础设施技能实现，缺少可选注入时
  使用空提供器，避免隐藏的配置回退。
- [x] Session runtime 和工作区 runtime 的生命周期契约已下沉到
  `agent/ports/sessions.py`、`agent/ports/workspace.py`；application 不再导入 Harness
  owner，stores 不再读取基础设施路径，缺少组合注入时执行入口明确失败。
- [x] 受影响行为回归 `495 passed`，架构守卫 `74 passed`，并通过导入图、`compileall`
  和 `git diff --check`。下一切片进入重组后的 application/harness 公开 API 收口与
  历史包消费者审计；不恢复旧平铺兼容 facade。
...
| `agent/adapters/protocol/client.py -> protocol.client.chat` | 统一模型流、attach/replay 和审批恢复传输原语 | `protocol.client` 接管所有调用；adapter 只保留本地坐标、游标和 Item 投影 | 4B/5 |
...
| `mind_app/runtime/environment/snapshot.py` -> `agent/application/turns/environment.py`、`mind_app/interaction/environment.py` | 环境能力采集用例与 Controller/Helix 上下文适配分离 | 环境/入口回归通过，旧模块删除，application 边界守卫通过 | 5 |
| `mind_app/runtime/hooks/models.py` -> `agent/application/hooks/models.py` | Hook 生命周期快照、决定、输出和工具结果值对象归入 application contract；执行器/注册器仍由 runtime 持有 | Hook/工具/Turn/TUI 定向回归 `421 passed`，旧模块/旧导入守卫和 application 边界守卫通过 | 5 |
| `mind_app/runtime/hooks/protocol.py` -> `agent/application/hooks/protocol.py` | Hook stdin/stdout schema、构建和校验归入 application boundary；不与线上 `protocol/` 混淆 | Hook 协议/架构定向回归 `31 passed`，旧模块/旧导入及 application 边界守卫通过 | 5 |
| `mind_app/runtime/hooks/catalog.py` -> `agent/application/hooks/catalog.py` | Hook 管理目录、状态快照和内容冲突归入 application contract | Hook 目录/TUI/架构定向回归 `22 passed`，旧模块/旧导入及 application 边界守卫通过 | 5 |
...
| `mind_app/runtime/hooks/events.py` -> `agent/application/hooks/events.py` | Hook 生命周期事件规格和目录一致性归入 application contract | Hook 事件/架构定向回归 `32 passed`，旧模块/旧导入及 application 边界守卫通过 | 5 |
| `mind_app/runtime/hooks/effects.py` -> `agent/application/hooks/output.py` | Hook 输出语义校验、决定归一化和业务阻断归入 application contract | Hook 输出/架构定向回归 `32 passed`，旧模块/旧导入及 application 边界守卫通过 | 5 |
...
| `mind_app/runtime/hooks/results.py` -> `agent/application/hooks/result.py` | 后置 Hook 工具结果替换、反馈和上下文投影归入 application contract | Hook 工具/架构定向回归通过，旧模块/旧导入及 application 边界守卫通过 | 5 |
...
| `mind_core/agent_config.py`、`feature_config.py` -> `agent/application/config/settings.py` | Agent 运行设置与能力开关归入 application | application 公开入口、配置/设置/Subagent 回归通过，旧源模块删除 | 5 |
...
| 2026-08-30 | 阶段 5 应用设置/Provider 配置切片 | 将 Agent/Feature 设置合并到 `agent/application/config/settings.py`，将 Provider 规则迁入 `infrastructure/config/providers.py`，切换所有消费者并删除三个旧模块 | 设置/配置服务/Subagent 定向测试 `244 passed`，架构测试 `15 passed`，全量回归 `2967 passed, 11 skipped`，`compileall` 和导入图检查通过；下一切片处理权限、项目策略、hooks/skills |
...
| 2026-08-31 | 阶段 5 环境快照用例切片 | 将环境能力调用与失败收敛从 `mind_app/runtime/environment/snapshot.py` 提取到 `agent/application/turns/environment.py`，将工作区/Helix 上下文聚合放入 `mind_app/interaction/environment.py`，切换 CLI/TUI/MCP/Subscription/Turn setup 并删除旧模块 | 环境与入口定向回归 `180 passed`，旧路径/旧导入及 application 边界守卫 `2 passed`，全量回归 `2995 passed, 11 skipped`，旧模块删除，导入图、`compileall` 和差异检查通过；下一切片继续审计交互、历史、客户端工具和完整入口边界 |
| 2026-08-31 | 阶段 5 Hook 运行模型切片 | 将纯 Hook 生命周期快照、决定、输出和工具结果值对象从 `mind_app/runtime/hooks/models.py` 重组到 `agent/application/hooks/models.py`，切换 runtime/TUI/工具/Subagent 消费者并删除旧模块 | Hook/工具/Turn/TUI 定向回归 `421 passed`，旧路径/旧导入及 application 边界守卫通过；全量回归 `2996 passed, 11 skipped`，导入图、`compileall` 和差异检查通过；下一切片继续审计交互、历史、客户端工具和完整入口边界 |
| 2026-08-31 | 阶段 5 Hook 协议边界切片 | 将 Hook stdin/stdout schema、事件输入构建和输出校验从 `mind_app/runtime/hooks/protocol.py` 重组到 `agent/application/hooks/protocol.py`，切换 scope/事件/效果消费者并删除旧模块 | Hook 协议/架构定向回归 `31 passed`，旧路径/旧导入及 application 边界守卫通过；全量回归 `2997 passed, 11 skipped`，导入图、`compileall` 和差异检查通过 |
| 2026-08-31 | 阶段 5 Hook 目录与匹配规则切片 | 将 Hook 管理目录从 `mind_app/runtime/hooks/catalog.py` 重组到 `agent/application/hooks/catalog.py`，将 matcher 与工具别名规则从 `matching.py` 重组到 `agent/domain/hook_matching.py`，切换 Controller/TUI/runtime 消费者并删除旧模块 | Hook 目录/TUI/执行/架构定向回归 `22 passed`，旧路径/旧导入及 application/domain 边界守卫通过；全量回归 `2999 passed, 11 skipped`，导入图、`compileall` 和差异检查通过 |
| 2026-08-31 | 阶段 5 Hook 输出与事件规格切片 | 将 Hook 输出归一化从 `mind_app/runtime/hooks/effects.py` 重组到 `agent/application/hooks/output.py`，将事件规格从 `events.py` 重组到 `agent/application/hooks/events.py`，切换 runtime/协议测试消费者并删除旧模块 | Hook 输出/事件/架构定向回归 `32 passed`，旧路径/旧导入及 application 边界守卫通过；全量回归 `3000 passed, 11 skipped`，导入图、`compileall` 和差异检查通过 |
...
| 2026-08-31 | 阶段 5 Hook 工具结果投影切片 | 将后置 Hook 工具结果替换、反馈、阻断和上下文投影从 `mind_app/runtime/hooks/results.py` 重组到 `agent/application/hooks/result.py`，切换 ToolHookEvents/工具测试消费者并删除旧模块 | Hook 工具/架构定向回归 `61 passed`，旧路径/旧导入及 application 边界守卫通过；全量回归 `3002 passed, 11 skipped, 34 warnings`，导入图、`compileall` 和差异检查通过 |
...
| 2026-08-31 | 阶段 5 上下文压缩结果与编排分离切片 | 将 `CompactResult` 提升到 `agent/application/turns/compact_result.py`，将 runtime 编排改名为 `mind_app/runtime/compaction.py`，删除旧 `runtime/conversation.py` 并补充结果/编排分离守卫 | 压缩/TUI/架构定向回归通过，导入图、`compileall` 和 `git diff --check` 通过；下一切片继续审计 runtime/subagents 和剩余平铺入口 |
| 2026-08-31 | 阶段 5 执行上下文契约归位切片 | 将 `AgentContext`、`TurnContext`、`ToolInvocation` 提升到 `agent/application/turns/context.py`，切换 Turn/工具/Hook/MCP/子 Agent/TUI 全部消费者并删除旧 `runtime/execution.py` | 执行上下文及能力定向回归 `230 passed, 1 warning`，application 归属与旧导入守卫、导入图、`compileall` 和 `git diff --check` 通过；下一切片继续收敛子 Agent mailbox/graph 边界 |
| 2026-08-31 | 阶段 5 子 Agent mailbox 存储归位切片 | 将 mailbox 事件/快照/消费游标和有界日志从 `mind_app/runtime/subagents/mailbox.py` 重组到 `agent/stores/agents/mailbox.py`，切换控制/投递/图/运行时/客户端工具消费者并删除旧入口 | mailbox/子 Agent 定向回归 `53 passed, 1 warning`，stores 边界与旧导入守卫、导入图、`compileall` 和 `git diff --check` 通过；下一切片继续拆分 Agent graph 快照与 SQLite 持久化实现 |
...
| 2026-08-31 | 阶段 5 Agent graph 快照与 SQLite 存储归位切片 | 将 Agent 状态/任务值对象重组到 `agent/domain/agents.py`，将图快照、SQLite 存储和单写者持久化重组到 `agent/stores/agents/graph.py`，删除旧 runtime graph 入口并清理 control 重复定义 | Agent graph/control/runtime 与 stores/domain/旧导入守卫回归 `51 passed, 1 warning`，导入图、`compileall` 和 `git diff --check` 通过 |
...
| 2026-08-31 | 阶段 5 Hook 输入上下文归位切片 | 将 Hook 输入上下文值对象和权限模式映射重组到 `agent/application/hooks/context.py`，runtime scope 仅保留 dispatcher 与生命周期实现 | Hook/Turn/压缩/Subagent/TUI 定向回归 `247 passed`，application/runtime 守卫、导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Turn 执行契约归位切片 | 将 `TurnExecution` 提升到 `agent/application/turns/execution.py`，以 `HookExecutionScopePort` 解耦 application 与 runtime HookScope | Turn/RunResult/Stream/Subagent/TUI 与架构守卫回归 `108 passed`，导入图、`compileall` 和 `git diff --check` 通过 |
...
| 2026-08-31 | 阶段 5 Subagent Hook 生命周期归位切片 | 将 `SubagentHookEvents` 重组到 `agent/application/hooks/subagent.py`，扩展 Hook scope 端口并删除旧 runtime Hook 模块 | Hook/Subagent/Turn/架构守卫回归 `137 passed, 1 warning`，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 SubagentRunner Harness 化 | 将 SubagentRunner 重组到 `agent/harness/execution/subagent_runner.py`，将 continuation 构造归入 application，改用 Turn runner/cleanup 注入并消除 harness 包初始化循环 | Subagent/Turn/SessionLoop 回归 `65 passed`，Harness 归属守卫、导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Subagent 流式执行适配器归位 | 将 StreamSubagentExecutor 重组为 `agent/adapters/agents/execution.py`，新增 SubagentStreamPort 并删除旧 runtime executor | Subagent/Turn/工具/adapter 守卫回归 `61 passed`，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 AgentControl Harness 归位 | 将完整 AgentControl 状态机重组到 `agent/harness/agents/control.py`，切换 SubagentRuntime/client tools/TUI 并删除旧 runtime control | AgentControl/graph/Subagent/TUI 回归 `78 passed`，graph/control 归属守卫、导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Agent 活动投递状态归位 | 将 AgentActiveTurn/AgentMessageDispatch 重组到 `agent/harness/agents/delivery.py`，切换 SubagentRuntime/TUI/测试并删除旧 runtime delivery | Agent delivery/Subagent/TUI/架构守卫回归通过，导入图、`compileall` 和 `git diff --check` 通过 |
...
| 2026-08-31 | 阶段 5 Agent 只读视图归位 | 将 AgentSnapshot/AgentWaitResult/AgentMailboxWaitResult 重组到 `agent/application/agents/views.py`，Harness control 只保留可变状态机 | AgentControl/graph/Subagent/TUI/视图守卫回归通过，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Agent 根会话注册表归位 | 将 SubagentRuntime 根会话 control 字典、生命周期锁和 shutdown 状态重组到 `agent/harness/agents/registry.py`，以显式 factory 负责恢复/创建 | 注册表/AgentControl/Subagent/runtime/架构守卫回归通过，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 父会话继承上下文算法归位 | 将 ForkContextEntry、继承范围选择、渲染和字符预算算法重组到 `agent/application/agents/fork_context.py`，runtime context 收敛为 history adapter | Subagent/context/application/架构守卫回归通过，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Subagent 提交执行协调归位 | 将 SubagentRuntime `_execute_submission` 重组到 `agent/harness/execution/subagent_submission.py`，显式注入 control、Hook、执行、投递和权限端口 | Subagent/runtime/tools/delivery 回归与 Harness 守卫通过，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Agent 消息派发结果归位 | 将 AgentMessageDispatch 从 Harness delivery 重组到 `agent/application/agents/messages.py`，活动投递模块只保留可变状态 | 消息投递/Subagent/application/架构守卫回归通过，导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-08-31 | 阶段 5 Agent 职责化目录与生命周期端口重组 | 将 application/harness/stores/adapters 的平铺模块按 agents/turns/hooks/config、agents/execution/sessions、agents/runs/effects/approvals、protocol/agents 分组；新增 `agent/ports/sessions.py` 与 `workspace.py`，让 application 通过 factory 使用 Harness，stores 只接收已解析路径 | 受影响行为回归 `495 passed`，完整架构守卫 `74 passed`，旧路径/旧导入、导入图、`compileall` 和 `git diff --check` 通过 |
| 2026-09-01 | 阶段 5 Interaction 职责拆分与 TUI 执行端口注入 | 删除 `mind_app/interaction`：Session 状态迁入 `agent/harness/sessions`，前端输入迁入 `frontends/interaction`，环境聚合迁入 `infrastructure/services`；TUI execution/root session 由 CLI 组合边界显式注入；修正 `protocol -> agent` 稳定 ID 反向依赖 | 扩展回归 `1651 passed`，先行定向 `262 passed`，端口专项 `147 passed`；边界守卫、导入图通过，`frontends -> mind_app` 从 `17/21` 降至 `11/14` |
| 2026-09-01 | 阶段 5 MCP、Helix 与输出职责拆分 | 将 MCP 配置/传输/值/注册表迁入 `infrastructure/mcp`，Helix 环境聚合迁入 `infrastructure/services`，Helix 前端生命周期与输出净化迁入 `frontends`；终端轮次改用显式宿主协议，删除五个 legacy 源模块 | MCP/Helix `287 passed`，扩展 MCP `193 passed`，输出 `146 passed`，终端轮次 `107 passed`，基础设施契约 `3 passed`；架构专项通过，导入图 `frontends -> mind_app` 从 `11/14` 降至 `5/7` |
| 2026-09-01 | 阶段 5 TUI 根轮次与压缩用例收口 | TUI Turn 改为只消费组合根绑定的 `TuiRootTurnRunner`，删除 execution/session/model/tool/report 参数链；压缩改为注入 `ConversationCompactor`，前端不再导入 legacy runtime | TUI `65 passed`、CLI `121 passed`、application `505 passed`、domain/capability `179 passed`；完整架构扫描其余 `104 passed`，修正两项过期断言后专项 `14 passed`；`compileall`、导入图通过，`frontends -> mind_app` 从 `5/7` 降至 `3/3`，下一切片收口 CLI/MCP Controller 边 |
| 2026-09-01 | 阶段 5 前端应用宿主构造倒置 | CLI 与 stdio MCP 定义各自最小应用宿主 Protocol，具体 `Mind` factory 只由 `mind.py` 注入；测试改为显式 fake factory，全前端守卫禁止重新导入旧应用 | CLI/MCP/TUI/组合根/baseline `167 passed, 1 warning`，导入图、`compileall` 和差异检查通过；`frontends -> mind_app` 从 `3/3` 清零且双向环消失，下一切片处理 `mind_app -> frontends` 的 `2/7` 反向边 |
| 2026-09-01 | 阶段 5 前台轮次 application 用例归位 | 将通用执行/完成/清理顺序迁入 `agent/application/turns/foreground.py`，worked footer 通过扩展的 `TurnForegroundLifecyclePort` 由 terminal adapter 投影，root runtime 不再调用前端函数 | 展示、失败清理、TUI 活动和根轮次回归 `122 passed`，架构守卫、导入图和语法检查通过；`mind_app -> frontends` 从 `2/7` 降至 Controller 单文件 `1/6` |
| 2026-09-01 | 阶段 5 Controller 前端边界清零 | 新增 Frontend/Activity/Attachment 端口，具体附件、静默输出、审批 presenter 和 worked footer 由 `mind.py` 注入；删除 terminal lifecycle/animation adapter，并将 Helix 下载改为显式 `UpgradeProgress` | 生命周期 `121 passed`、CLI/TUI `236 passed`、MCP `14 passed`、Controller `49 passed`；完整架构 `104 passed`，修正两项过期断言后专项 `5 passed`；导入图中 `mind_app -> frontends` 从 `1/6` 清零且无跨边界循环 |
| 2026-09-01 | 阶段 5 四入口启动/恢复证据收口 | 在同一提交基线上分别复核 CLI Resume/失败恢复、TUI 启动门禁/backtrack、stdio MCP 会话续接和 Subscription 暂停续接/重启，不新增重复测试实现 | CLI `14 passed`、TUI `29 passed`、stdio MCP `14 passed`、Subscription `18 passed`；下一切片进入外部 MCP runtime/adapters 职责拆分 |
| 2026-09-01 | 阶段 5 外部 MCP SDK 与连接生命周期归位 | 将外部连接组、启动状态、异常分类、本地 SDK 会话和应用级 runtime 迁入 `infrastructure/mcp`，组合根与剩余会话适配器切换新入口并删除五个旧模块 | MCP group `11 passed`、TUI/运行入口 `95 passed`、工具链路 `63 passed`、架构专项 `3 passed, 2 warnings`；导入图、`compileall` 和 `git diff --check` 通过；下一切片先建立工具注册端口再迁移 session adapter/tool catalog |
| 2026-09-01 | 阶段 5 工具会话组合与构造权归位 | 新增工具注册表、外部工具组、动态来源和 runtime 端口；将 Composite session、tool catalog 和 tool runtime 迁入 `infrastructure/mcp`，删除四个只转发 provider，并由 `mind.py` 工厂注入 Controller | 工具会话 `26 + 81 + 35 passed`、Turn/Subagent `51 passed`、工具结果/权限 `70 passed`、CLI/TUI/清理 `39 + 123 passed`、架构专项 `5 passed, 2 warnings`；导入图、`compileall` 和差异检查通过；下一切片收口剩余 MCP 结果/目录/展示职责 |
| 2026-09-01 | 阶段 5 旧 MCP runtime 源目录退役 | 将 SDK 工具结果归一化迁入 `infrastructure/mcp`，工具目录查询迁入 `agent/application/tools`，进度支持规则迁入 domain 并把通知/观测内聚到唯一执行路由；删除最后三个旧模块 | 工具结果/进度/计划 `24 passed`、客户端工具链 `48 passed`、流式结果 `70 passed`、架构专项 `4 passed, 2 warnings`；旧路径扫描、`compileall` 和差异检查通过；下一切片按能力族重组 client/builtin tools |
| 2026-09-01 | 阶段 5 本地工具契约与注册表归位 | 将调用上下文和 client/builtin 定义迁入 `agent/application/tools`，以 `infrastructure/mcp/local_tool_registry.py` 统一 MCP schema 适配和注册状态，删除四个重复 types/registry 模块及包级兼容导出 | 工具/权限/Turn 回归 `270 passed`，TUI `1512 passed`，CLI/MCP `151 passed`，Subscription/输出/附件 `150 passed`；完整架构 `106 passed / 2 stale assertions`，修正后职责专项 `5 passed, 3 warnings`；导入图、`compileall`、差异检查和旧符号扫描通过 |
| 2026-09-01 | 阶段 5 本地工具结果与 planning 能力归位 | 新增不可变、JSON 校验的 `LocalToolResult`，所有本地 handler 退出 MCP SDK 返回类型，唯一 registry 适配 wire 结果并校验身份；将 plan_steps/update_plan 的 schema、校验和定义迁入 `agent/application/tools`，删除三个旧模块 | 规划/工具/Turn 回归 `230 passed`，终端展示 `145 passed`，职责专项 `5 passed, 2 warnings`；导入图、`compileall` 和旧路径扫描通过；下一切片迁移 media 与 permissions 能力 |
...
| 2026-09-01 | 阶段 5 Workspace JavaScript 与本地工具装配归位 | 将 REPL 用例、嵌套策略/审批迁入 application，以工作区端口执行；MCP adapter 统一转换并校验嵌套 JSON 结果，基础设施统一装配客户端/内置注册表，删除整个旧 `mind_app/client_tools` 源包 | 联合回归 `172 passed`；完整架构 `107 passed / 1 stale assertion`，修正后职责节点 `3 passed, 2 warnings`；导入图、`compileall` 和差异检查通过；下一切片重组 workspace 编码具体实现 |
| 2026-09-01 | 阶段 5 Workspace 编码实现整体归位 | 将纯补丁模型/解析/delta 拆入 domain，将 context、补丁执行/差异、命令审计/持续会话、用户 Shell、REPL 池和聚合实现重组至 `infrastructure/workspace`；改名 `WorkspaceCoding` 并删除整个旧 `mind_app/native_coding` 源包及两个死 helper | 工作区联合回归 `169 passed`，职责/legacy 清零守卫 `5 passed, 3 warnings`；导入图、`compileall`、旧导入扫描和差异检查通过；下一切片重组工具执行编排 |
| 2026-09-02 | 阶段 6 工具展示、MCP 执行与结果增强归位 | 将工具开始/结果/进度投影归 application，将 MCP SDK 调用、结果归一化和 Hook 响应适配归 MCP infrastructure，将远端 heal 增强归 services；删除旧 display/progress/enhancement/router/run 共九个源码模块 | 工具执行联合回归 `128 passed`，职责守卫 `3 passed, 1 warning`；导入图、`compileall`、旧导入扫描和差异检查通过；下一切片迁移 client effect/Hook 与 plan 生命周期 |
| 2026-09-02 | 完成工具执行编排归位：建立 SDK-free `ToolExecutionAdapter`，迁移客户端工具、计划和工具 Hook 生命周期，删除旧 runtime 工具源码包 | 工具/计划/嵌套 `82 passed`；Turn/Subagent `117 passed`；入口 `151 passed`；职责守卫 `5 passed, 1 warning`；导入图、`compileall`、差异检查通过 |
| 2026-09-02 | 完成 Hook 领域生命周期归位：迁移 Compact/Session/Turn/展示编排，拆出纯 view builder 并删除旧 Hook 源包 | Hook/压缩/流式 `167 passed`；职责守卫 `4 passed`；导入图、`compileall`、旧导入扫描和差异检查通过；下一切片收敛旧 Turn 运行时 |
| 2026-09-02 | 完成 Turn 协议边界第一组：模型事件与工具结果归 protocol adapters，终态展示归 application，Harness/Subagent 改用 `EventReportPort` | 定向 `88 passed`；扩展主链 `326 passed`；全量架构 `109 passed / 4 stale assertions`，修正后相关 `5 passed, 2 warnings`；导入图、`compileall`、旧导入扫描通过；下一切片拆分审批、策略和工具事件边界 |
| 2026-09-02 | 完成 Turn 协议边界第二组：执行策略结果归 domain，本地审批规则归 application，审批/工具事件归 protocol adapters，hosted 输出通过工具执行端口投影；finalizer 改用 Transcript 生命周期端口 | 策略/权限/协议 `144 passed`；扩展主链 `186 passed`；Transcript `20 passed`；职责守卫 `5 passed, 1 warning`；`compileall`、旧路径和迁移范围强制类型声明扫描通过；下一切片收敛 setup/finalize 与主循环 |
| 2026-09-02 | 完成 Turn 协议边界第三组：setup 归 protocol adapter、finalizer 归 Harness、Turn transcript 事实归 application，并以空闲计时器端口隔离具体平台 timer | setup/finalize/Transcript/主链 `109 passed`；职责守卫 `4 passed, 1 warning`；`compileall`、旧路径扫描通过；下一切片收敛 executor/root/stream 主链 |
| 2026-09-02 | 完成 Turn 执行编排归位：报告租约、MCP 会话、工具过滤和 setup 失败收束迁入 Harness，以生命周期和 Hook scope provider 端口消除具体 Protocol Client/Controller 依赖，删除旧 executor | Turn/stream/Protocol/Subagent `204 passed`；职责守卫 `4 passed, 2 warnings`；`compileall`、旧导入扫描通过；下一切片拆分 stream 主循环 |
| 2026-09-02 | 完成 Turn 协议流整体归位：拆分模型请求、中断命令和重试状态，效果账本路径绑定组合根，idle 调度归 Harness；删除旧 stream、未使用宿主参数和旧 Subagent 适配器 | 主流 `215 passed`；Controller/根 Turn/Subagent `144 passed`；职责守卫 `7 passed, 2 warnings`；`compileall`、旧导入扫描通过；下一切片迁移根 Turn 与 Controller 会话适配器 |
| 2026-09-02 | 完成根 Turn 与宿主端口收口：根 runner 迁入 Harness，Controller 直接履约当前状态端口，删除四个转发 facade 并清空旧 runtime/turns 源目录 | 根/流/Controller/Subagent `144 passed`；四入口 `140 passed`；职责守卫、`compileall`、旧导入扫描通过；下一切片收敛 compaction 与 Controller |
| 2026-09-02 | 完成 Conversation Compaction 职责化迁移：wire 事件归 Protocol adapter，Hook/Transcript/取消归 Harness，具名事件/结果与 Session/Client ports 分层；删除旧 runtime 源目录 | 压缩/Hook/根 Turn `135 passed, 1 warning`；职责守卫、`compileall`、旧导入扫描通过；下一切片拆分最终 Controller |
