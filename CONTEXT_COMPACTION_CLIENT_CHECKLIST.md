...
- 自动压缩完成记录与接续正文之间不插入横线。完成记录收束此前工作段，即使此前有工具执行或压缩后直接结束 Turn，也不再补画该段的横线；此后新工具建立自己的正常工作段。
...
- [x] 完成后直接继续 Thinking/正文，不因压缩插入横线；无重置 Turn elapsed 或错误释放输入屏障的问题。
...
### 完成后不插线（2026-09-14 后续设计修正）

按最新要求，自动压缩完成记录直接收束此前工作段，清除待补画的横线；下一段回复或 Turn 结束均不再为该段插线。已移除旧的压缩完成重新触发横线入口。失败和手动展示不改写当前 Turn 的工作边界，压缩后新执行的工具仍建立正常工作段。

新增 pre_turn/mid_turn × 此前是否已回复 × 是否继续正文的 8 组回归，修改前全部复现插线，修改后通过。压缩展示、工具布局、renderer 帧契约与手动压缩合计 137 passed；compileall、git diff --check 通过。本次属于客户端展示规则修正，未重跑上文真实服务矩阵。
