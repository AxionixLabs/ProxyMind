...
- [ ] 网络暂时不可用时给出可理解的错误，恢复网络后可重试。
- [x] 使用测试账户在服务端撤销授权后，客户端识别失效并提示重新登录，不进入无限刷新
  或自动弹窗循环。独立 Sentry 授权通过正式撤销端点撤销，下一只读调用拒绝且目录撤下，
  活动源码 TUI 重启后显示重新授权，资源关闭通过。
...
| 浏览器启动失败后的手动恢复、网络故障 | 待验收             | 本轮手动访问成功；尚未制造应用浏览器启动失败或真实断网 |
| 服务端撤销授权                      | 通过               | 独立 Sentry 授权撤销后实际调用拒绝、目录清空，TUI 提示重新登录 |
...
阶段六仍未全部完成：浏览器启动失败后的手动恢复、真实断网、刷新交换中的退出竞争及
Linux/macOS 保持待验收。Windows 本轮证据、失败尝试及复验结果均保留在上述报告目录。
两个隔离验收注册已 logout 后 remove；本地验收凭据已清除，脱敏报告保留。

阶段七补充证据位于 `build/mcp-maturity-phase7-20260917/ACCEPTANCE.md`：真实 Sentry
服务端撤销、新进程恢复、TUI 服务控制和退出检查通过，本轮独立注册及凭据已清理。
本轮完整模型回合返回服务器过载，未触发审批；不替代或扩大此前审批真机证据。
