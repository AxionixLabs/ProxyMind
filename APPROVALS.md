...
- [x] 服务端权威事件顺序中 completed 不得先于 matching started，完成时间不得早于开始时间。
  客户端允许因保留窗口裁剪而只收到自包含的 completed，并直接归约该终态。
- [x] 对应 `tool.approval_required` 必须在 completed 之后发布，使客户端可在不阻塞
...
- [x] 评审活动使用独立 `review_id` lease，不能借用交互审批卡的 `approval_id` lease。
- [x] 并行评审按开始顺序稳定显示；更新同一 `review_id` 不改变位置。
- [x] 活动明细不显示风险、解释或内部模型信息，只显示待评审动作摘要。
- [x] 独占审批卡可以暂时遮挡活动区，但不能修改评审 reducer 状态。
- [x] `presentation.superseded`、Turn 终态、输出会话关闭必须清除陈旧评审 lease。
- [x] attach/replay 期间只归约状态，追平后一次性投影，不重启动画计时。
- [x] `codex-main` 评审结束回落到 `Working`；ProxyMind 已统一基础活动语义，故回落到
  当前状态机推导的 `Thinking`、`Retrying` 或隐藏状态，不重新引入 `Working`。
- [x] `Reviewing` 属于等待动画标题族，保持常规字重和现有扫光规则，不显示工具名称。
...
- [x] `{APP_NAME}` 必须来自 `metadata.const`，不得硬编码产品名。
- [x] `denied` 与 `timed_out` 必须是不同展示状态，不能都退化为普通 decline 文案。
- [x] 命令、写入 stdin、补丁、网络、权限和 MCP 分别生成结构化动作摘要。
...
- [x] 给 activity reducer 增加评审 lease 和单项/并行投影。
- [x] TUI 实现 `Reviewing approval request(s)` 标题和动作明细。
- [x] 自动通过保持静默；拒绝、超时使用独立的 warning 与动作记录。
- [x] text/JSONL 输出保留可机器消费的评审终态，不依赖 TUI 文案。
- [x] 验收：所有前端从同一 application view/activity 语义投影，不重复实现状态判断。

阶段 4 证据：审批、协议、主流、TUI、terminal、text 与 JSONL 定向回归共
578 项通过；`python -m compileall -q agent protocol frontends` 与
`git diff --check` 通过。服务端正式契约位于提交 `278b3fd`，对应契约测试
`tests/test_approval_review_contract.py` 共 11 项通过。
