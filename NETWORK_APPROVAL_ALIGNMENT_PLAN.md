...
| --- | --- | --- | --- | --- |
| 1 | 审批领域契约、状态所有权和迁移准入 | 大 | 待开始 | 目标审批架构冻结 |
| 2 | 通用审批核心落地并迁移现有审批 | 特大 | 待开始 | 非网络审批使用新核心 |
| 3 | 三平台静态受管网络 | 特大 | 待开始 | 静态网络规则可执行 |
| 4 | 网络决定、规则提交和审批卡 | 特大 | 待开始 | 前台网络审批闭环 |
...
- 架构守卫证明没有新 `Mind` 领域命名、`typing.cast()`、`Any` 扩大或兼容 fallback。
- 现有审批测试保持不变通过。
