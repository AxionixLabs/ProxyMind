...
服务端事件发布、历史裁剪后的权威快照恢复交付和 P4 真实服务/终端验收仍待配套，不能据此
将 P0–P5 或完整对齐标为完成。
...
- [ ] 服务端正式目录和信封登记 `context.usage.updated`；客户端已固定名称和严格解析，等待服务端配套。
...
| 协议和测试替身 | 定向测试通过，覆盖严格解析、重复与旧事件、会话隔离、缓存恢复与裁剪、手动压缩资源关闭及 footer 布局；TUI runtime 174 项通过 |
...
| 真实服务和终端 | 未验收；当前证据包含进程内 prompt_toolkit 帧与样式属性测试，不能替代 P4 的真实服务连接、输入法和拖动窗口验收 |
