...
压缩后的 token 占用由终态前的独立用量事件交付。默认状态栏只在 Turn 运行中且输入框有
待排队草稿时显示右侧上下文余量，左侧为 `tab to queue message`；空闲和空输入时隐藏用量。
Codex 默认启用模型与目录状态栏，不能将禁用状态栏时的回退快照当作默认显示逻辑。
显示条件见 `codex-main/codex-rs/tui/src/bottom_pane/footer.rs` 的
`shows_passive_footer_line`、`uses_passive_footer_status_layout`，以及
`chat_composer.rs` 的 `show_queue_hint` 和 `right_line` 分支；默认状态栏配置见
`chatwidget.rs` 的 `DEFAULT_STATUS_LINE_ITEMS`。
...
- [x] 定义 `initial`、`pending`、`known`、`unknown` 四种本地展示状态：initial 候选值为 100%，空闲首屏不显示；恢复未完成时隐藏；已知时在允许的 footer 模式显示比例或绝对用量；无可靠值的旧会话不伪装成新会话。
...
- [x] 在已有 footer 行内右对齐，距终端物理右边缘固定两列，并与左侧至少间隔一列；明确扣除输出后端已有的保留列，不扩大安全写入宽度。
- [x] 将 C7 的默认状态栏与排队模式分开：普通模式只显示既有状态栏；Turn 中有草稿才尝试完整和简短排队提示及用量，仍放不下时隐藏 context。
- [ ] 历史搜索、退出确认、回看提示、菜单及隐藏模式遵守既有优先级，不能叠字。Shell 模式和未来自定义 status line 也须有唯一右侧内容来源。
- [x] 为百分比预留稳定列宽，位数变化和恢复隐藏不切换排队提示版本；仅在最终可见片段改变时重绘，不增加逐 token 估算或后台请求。
...
| 扩大回归       | 已修正 `test_observed_replay_does_not_repeat_completed_client_tool` 的旧断言：正式恢复流程包含 replaying、工具结果、caught_up 三个批次；继续验证不重复执行工具、不重复提交结果。该模块 6 项通过，本轮客户端定向回归 55 项通过                |
...
| 真实断线与重接         | 在实际 tool.call 后关闭一次 `/mind-chat` 连接，走 reconnecting → replaying → caught_up；恢复期间 footer 隐藏，完成后恢复。整个 Turn 只提交一次 `/mind-chat`、一次 `/mind-attach`，最终得到 56088                                                                                                                    |
| 历史前缀清理           | 删除验收会话 seq 1–12，并将保留起点设为 13；清除本地用量缓存后，冷恢复仍取得独立快照 seq 12、14364 → 97%。没有模型提交或默认 100% 闪现                                                                                                                                                                              |
| 不同模型窗口           | 隔离配置下真实切换 gpt-5.6-sol/100000 与 qwen3.8-max/80000；QWEN 返回 provider 9416 → 100%。切换配置不改写旧快照的模型、窗口和用量，下一次事件整体替换                                                                                                                                                              |
| Review 与冷恢复        | 复现并修复 inline Review 覆盖服务端主会话快照的问题。部署后主会话 estimate 14208 → 97%，Review 用量依次为 1767、4255、10041，主会话仍保持 seq 7。清除本地缓存后冷恢复为 97%，没有 100% 闪现或模型提交                                                                                                               |
| 实际压缩失败           | QWEN 手动压缩返回 summary_failed；保留此前 provider 9416/80000 的快照，没有伪造新的用量。尚未完成确定性故障注入及该 provider 摘要失败原因定位                                                                                                                                                                       |
| 终端宽度与样式         | 原生终端列数 120、70、30、20，prompt_toolkit 可用列数 119、69、29、19；可见时单行右对齐并保留一列边距，19 列隐藏，放宽后恢复；实际解析样式 dim、非 bold                                                                                                                                                             |
| 默认显示时机与稳定边距 | 修正为 Codex 默认状态栏行为后，原生 Windows 验证空闲、运行中空输入及 Turn 结束后均隐藏；运行中输入草稿才显示排队提示和用量。实际 99% → 98% 保持右边缘；120、70、34 列均距物理右边缘两列，25 列隐藏，放宽恢复。渲染回归覆盖 100%、99%、9%、0% 的位数变化、pending、两类后端保留列和输入光标位置；相关回归 209 项通过 |
...
Review 快照隔离的真实 PostgreSQL 回归在旧实现上复现两项失败；修复后数据库与 schema
检查 32 项通过，事件、Review 及架构回归 209 项通过。已部署版本为
`20260912044153-580d1437bf59-3b9e3a10b31c`，服务健康检查通过。用户日常窗口配置保持
100000；80000 仅用于隔离验收配置。

尚未完成的真实场景包括服务端收窄同模型窗口、受控 provider 缺失 usage、确定性压缩失败、
detached Review、子代理、按指定历史边界 fork、中文输入法及实际按 Tab 提交排队消息；对应单元测试不能
替代这些真机项目。菜单、历史搜索、退出确认和 Shell 等 footer 模式尚未完整逐项验收。
